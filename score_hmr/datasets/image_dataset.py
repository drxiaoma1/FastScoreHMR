...
class ImageDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # For mpi_inf_3dhp dataset we only use S1&S2, if you use the full dataset, comment the code below
        if "mpi_inf_3dhp" in dataset_file.lower():
            valid_indices = [i for i, name in enumerate(self.imgname) if name.startswith('S1/') or name.startswith('S2/')]
            self.imgname = self.imgname[valid_indices]
            self.center = self.center[valid_indices]
            self.scale = self.scale[valid_indices]
            self.body_pose = self.body_pose[valid_indices]
            self.has_body_pose = self.has_body_pose[valid_indices]
            self.betas = self.betas[valid_indices]
            self.has_betas = self.has_betas[valid_indices]
            self.keypoints_2d = self.keypoints_2d[valid_indices]
            self.keypoints_3d = self.keypoints_3d[valid_indices]
            print(f"Loaded {len(self.imgname)} samples after filtering S1/S2")
