"""
Code borrowed from https://github.com/statho/ScoreHMR/blob/master/score_hmr/trainer.py
Tensorboard logging doesn't work yet.
"""
...
class Trainer(object):
    ...
    def save(self, milestone: int) -> None:
        ...
        torch.save(data, os.path.join(self.ckpt_folder, f"model-shortcut-test-{milestone}.pt"))
    ...
    def load(self, milestone: int, name: str = 'score_hmr', use_default: bool = False) -> None:
        ...
        data = torch.load(f"{ckpt_dir}/model-shortcut-test-{milestone}.pt", map_location=self.device)
        self.step = data["step"]
